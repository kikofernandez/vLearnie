...
from phpsessionencode.PHPObject import PHPObject
"""
...
class PHPUnserialize(object):
    ...
    def _unserialize(self, data, offset=0):
        ...
        # s => String
            ...
            readdataobj = {}
            #print "Estoy en O"
            # get the (dataoffset_value, name, # elements)
            # data[dataoffset:].split('{')[0] this gives me the values that I want as a string
            # .split(':') gives me the values in a list, but something like this: 6:object:2:
            # will return ['6', 'object', '2', ''] and using the [:-1] removes the last one
            current_str = data[dataoffset:].split('{')[0]
            (chars, key_obj, number_attributes) = current_str.split(':')[:-1]
            #print "key_obj: %s" % key_obj[1:len(key_obj)-1]
            
            #chars = int(chars)
            dataoffset += len(current_str) + 1
            
            for i in range(0, int(number_attributes)):
                #print data[dataoffset:]
                (ktype, kchars, key) = self._unserialize(data, dataoffset)
                #print "Tipos (%s, %s, %s)" % (ktype, kchars, key)
                dataoffset += kchars
                #print data[dataoffset:]
                
                (vtype, vchars, value) = self._unserialize(data, dataoffset)
                #print "Tipos (%s, %s, %s)" % (vtype, vchars, value)
                dataoffset += vchars
                
                readdataobj[key] = value
            
            dataoffset += 1
            #php = "%s%s(%s)" % (STR_PREFIX, str(key_obj[1:len(key_obj)-1]).capitalize(), readdataobj)
            key_obj = key_obj[1:len(key_obj)-1]
            phpobj = PHPObject(key_obj, **readdataobj)
            #readdata = {key_obj: php}
            readdata = phpobj
        ...
        #print "typeconvert readdate: %s %s" % (readdata,typeconvert(readdata))
