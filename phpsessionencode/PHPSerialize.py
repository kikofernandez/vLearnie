...
class PHPSerialize(object):
	...
	def serialize_value(self, data):
		...
		# @KikoFernandez:
		# It seems that Moodle keeps some numbers as strings and therefore
		# they must be serialize after unserializing. This is in order to 
		# maintain the same data format.
		elif type(data) is types.StringType:
			#if self.is_int(data):
			#	return "i:%s;" % int(data)
			#else:
		...
		elif type(data) is PHPObject:
			out = []
			for k in data.param.keys():
				out.append(self.serialize_key(k))
				out.append(self.serialize_value(data.param.get(k)))
			return "O:%i:\"%s\":%i:{%s}" % (len(data.name), data.name, len(data.param.keys()), "".join(out))
